...
namespace EventStreams.Projection {
    ...
    internal sealed class DefaultProjector : IProjector {
        ...
        public TModel Project<TModel>(IEnumerable<IStreamedEvent> events, Func<TModel, EventHandler> eventHandlerFactory)
            where TModel : class {

            return Project(Guid.NewGuid(), events, eventHandlerFactory);
        }

        public TModel Project<TModel>(Guid identity, IEnumerable<IStreamedEvent> events, Func<TModel, EventHandler> eventHandlerFactory)
            where TModel : class {
