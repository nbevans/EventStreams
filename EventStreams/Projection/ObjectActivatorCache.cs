...
namespace EventStreams.Projection {
    ...
    internal sealed class ObjectActivatorCache<T> where T : class, new() {

        private delegate object ObjectActivator(params object[] args);

        private static Func<Guid, T> _cachedCtor;

        public Func<Guid, T> Activator() {
            return _cachedCtor ?? (_cachedCtor = GetActivator());
        }

        private Func<Guid, T> GetActivator() {
            var arCtorInfo =
                typeof (T)
                    .GetConstructors()
            ...
            if (arCtorInfo == null)
                throw new InvalidOperationException(
                    string.Format(
                        "The type '{0}' does not define a constructor that accepts a '{1}' type.",
                        typeof (T), typeof (Memento<>)));
            ...
            return arId => {
                ...
                return (T)aggregateRootActivator(memento);
            };
        }
